...
public class SteamVR_Camera : MonoBehaviour
{
	...
	public void SetViewPlanes(float fNearZ, float fFarZ)
	{
		...
		foreach (var eye in eyes)
		{
			...
			for (int i = 0; i < 4; i++)
				for (int j = 0; j < 4; j++)
					m[i, j] = proj.m[i * 4 + j];
			eye.GetComponent<Camera>().projectionMatrix = m;
		}
	}
	...
	void OnEnable()
	{
		...
		var depth = eyes[0].GetComponent<Camera>().depth;
		...
		GetComponent<Camera>().depth = depth + 2000;
		...
		GetComponent<Camera>().farClipPlane = 1;
	...
	void Update()
	{
		...
		GetComponent<Camera>().enabled = applyDistortion;
	}
